...
## Private Backblaze B2 assets on Render

OpenParty can stream assets from a private B2 bucket through your server. The B2 application key is used only by Render; clients receive URLs such as `https://your-service.onrender.com/assets/public/map/...` and never receive the B2 key or authorization token.

1. In Backblaze, create an **application key** restricted to the one asset bucket (and, if applicable, its folder prefix). Give it the `readFiles` capability only. Do not use your master key.
2. In Render, open the web service's **Environment** page and add the values shown in [`.env.example`](.env.example): `B2_KEY_ID`, `B2_APPLICATION_KEY`, and `B2_BUCKET_NAME`.
3. Set `ASSET_PUBLIC_URL` to the public HTTPS URL of the Render service. Set `B2_FILE_PREFIX` only if all asset objects live below one folder.
4. Set `ASSET_PROXY_ORIGINS` to the old asset hosts appearing in your song/package JSON. For example, use `https://jd-s3.bc5f39.workers.dev` for the sample URLs in this repository. The object key in B2 must match the path after that host; `https://old-host/public/map/a.zip` maps to `public/map/a.zip` in B2.
5. Deploy/restart the Render service. URLs whose host is included in `ASSET_PROXY_ORIGINS` are automatically rewritten to `/assets/...` in the server responses.

The proxy forwards `Range` requests to B2. A private bucket remains private at Backblaze; anyone who can reach a valid public `/assets/...` URL can still retrieve that particular asset through your game server, so use a separate bucket or a restrictive `B2_FILE_PREFIX` for this service.
